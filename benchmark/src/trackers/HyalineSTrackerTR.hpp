...
#ifndef HyalineSTR_TRACKER_HPP
#define HyalineSTR_TRACKER_HPP
...
template<class T> class HyalineSTRTracker: public BaseTracker<T>{
private:
	int task_num;
	bool collect;
...
private:
	...
	static HyalineSTRTracker *myself;
public:
	~HyalineSTRTracker(){};

	HyalineSTRTracker(int task_num, int epochFreq, int emptyFreq, int slots, bool collect):
	...
	void* alloc(int tid){
		...
		lfbsmr_init_node(smr, (struct lfbsmr_node *) (node + sizeof(T)), &taskData[tid].counter, SMR_FREQ);
		return node;
	}
	...
	void end_op(int tid){
		lfbsmr_trim(smr, &taskData[tid].enter_num, SMR_ORDER, &taskData[tid].handle, free_node, 0, LF_DONTCHECK, FLUSH_THRESHOLD);
	}
...
template<class T>
HyalineSTRTracker<T>* HyalineSTRTracker<T>::myself;
