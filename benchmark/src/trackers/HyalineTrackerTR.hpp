...
#ifndef HyalineTR_TRACKER_HPP
#define HyalineTR_TRACKER_HPP
...
template<class T> class HyalineTRTracker: public BaseTracker<T>{
private:
	int task_num;
	bool collect;
...
private:
	...
	static HyalineTRTracker *myself;
public:
	~HyalineTRTracker(){};

	HyalineTRTracker(int task_num, int epochFreq, int emptyFreq, int slots, bool collect):
	...
	void end_op(int tid){
		lfsmr_trim(smr, taskData[tid].enter_num, SMR_ORDER, &taskData[tid].handle, free_node, 0, LF_DONTCHECK, FLUSH_THRESHOLD);
	}
...
template<class T>
HyalineTRTracker<T>* HyalineTRTracker<T>::myself;
